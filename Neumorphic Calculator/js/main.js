...
//now we are going to loop through all the number keys and put an event listener on them
...
//element on click--another arrow function
        ...
        el.innerText);                       //if the element is zero then we want to replace it with the elements number or inner text
...
//this is the specific call back
const opCallback = opName => () => {
    let currentVal = parseFloat($input.value);      //this converts the text to a number

    if (opName === "percent") {                     //this is distinguishing which operator
    ...
    else {                                          //if its not a percentage then we are going to add it to the buffer
        if (buffer && buffer.length) {              //if there is a value and operation in the buffer
            buffer.push({ value: currentVal});      //enter another value to the buffer

            const result = evaluate(buffer);         //performs the operation of any number of entries

            buffer.push({ value: result });         //displays the result of the last entry
            buffer.push({ value: opName });         //enter the next operation to be taken, if any

            $input.value = "";                      //clears the input display
        }
        else {
            buffer.push({ value: currentVal });     //puts whatever is in the buffer with the operation
            buffer.push({ value: opName });         //takes the operation selected
            $input.value = ""                       //clears the input display
        }
    }
}
...
for (const opName of ["add", "subract", "multiply", "divide", "percent"]) {     //we are looping through the array of operations
